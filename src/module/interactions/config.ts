    ...
    async execute(): Promise<void> {
        ...
        if (this.interaction.options.getSubcommandGroup(false) == "mention") {
            if (this.interaction.options.getSubcommand() == "status") {
                const embed = new MessageEmbed()
                    ...
                    .setDescription(`** : ${serverConfig.mention_role ? `<@${serverConfig.mention_role}>` : await this.localeMessage("NOTHING")}**`);
                this.interaction.reply({embeds: [embed]});
            }
        ...
        if (this.interaction.options.getSubcommand() == "list") {
            const embed = new MessageEmbed()
                ...
                .addField(await this.localeMessage("MESSAGE_CURRENT_ADMINROLE"), serverConfig.admin_role ? `<@&${serverConfig.admin_role}>` : await this.localeMessage("NOTHING"))
                ...
                .addField(await this.localeMessage("MESSAGE_CURRENT_MENTION"), serverConfig.mention_role ? `<@&${serverConfig.mention_role}>` : await this.localeMessage("NOTHING"));
            this.interaction.reply({embeds: [embed]});
        }
