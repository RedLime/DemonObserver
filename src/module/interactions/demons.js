...
export default class DemonsCommand extends CommandUserInteraction {
    async execute() {
        ...
        const [[demonCounts]] = await this.connection.query('SELECT COUNT(case when difficulty=0 then 1 end) as `hard`, '
        ...
        const result = async (description, easy, medium, hard, insane, extreme, total) => {
        ...
        if (!type) {
            ...
            const toChangeString = (target) => target.change == 0 ? '-' : (target.change > 0 ? this.emojis.UP : this.emojis.DOWN) + Math.abs(target.change);
            const [diffChanges] = await this.connection.query('SELECT type, data1, data2 FROM `gd_changelogs` WHERE log_timestamp >= DATE_SUB(NOW(), INTERVAL 7 DAY)');
            ...
            //레이팅 종류
            const [demonCounts] = await this.connection.query('SELECT creator_points, COUNT(case when difficulty=0 then 1 end) as `hard`, '
            +'COUNT(case when difficulty=3 then 1 end) as `easy`, COUNT(case when difficulty=4 then 1 end) as `medium`, '
            ...
            //길이
            const [demonCounts] = await this.connection.query('SELECT level_length, COUNT(case when difficulty=0 then 1 end) as `hard`, '
            +'COUNT(case when difficulty=3 then 1 end) as `easy`, COUNT(case when difficulty=4 then 1 end) as `medium`, '
            ...
            //업로드된 버전
            const [demonCounts] = await this.connection.query('SELECT ingame_version, COUNT(case when difficulty=0 then 1 end) as `hard`, '
            +'COUNT(case when difficulty=3 then 1 end) as `easy`, COUNT(case when difficulty=4 then 1 end) as `medium`, '
